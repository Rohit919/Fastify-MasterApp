...
function trustedProxiesFromEnv(): string[] | false {
  const configured = process.env.TRUST_PROXY?.split(",")
...
export async function buildApp() {
  const app = Fastify({
    ...
    // Never trust arbitrary X-Forwarded-* headers. Deployments must explicitly
    // list their load-balancer/proxy addresses in TRUST_PROXY.
    trustProxy: trustedProxiesFromEnv(),
