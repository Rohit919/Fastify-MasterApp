...
const ipKey = (prefix: string) => (req: FastifyRequest) => `${prefix}:${req.ip}`;
...
const recoveryRoutes: FastifyPluginAsyncTypebox = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      const { email, otp: code } = request.body as { email: string; otp: string };
      ...
      if (!result.ok) {
        throw new ValidationError('Invalid or expired verification code.', undefined, ErrorCode.OTP_INVALID);
      }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!result.ok || !result.userId) {
        throw new ValidationError('Invalid or expired reset code.', undefined, ErrorCode.OTP_INVALID);
      }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!result.ok) {
        throw new ValidationError('Invalid or expired reset token.', undefined, ErrorCode.TOKEN_INVALID);
      }
      ...
      fastify.log.info({ userId: result.userId }, 'auth.password_reset_completed');
