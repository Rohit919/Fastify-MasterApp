...
import { buildTestApp, signTestToken, TEST_ENV } from '@core/testing/test-app.js';

const BASE_URL = '/api/v1/auth';
const SECRET = TEST_ENV.JWT_SECRET;

const hmac = (code: string) => createHmac('sha256', SECRET).update(code).digest('hex');
...
describe('POST /api/v1/auth/password-reset/confirm', () => {
  ...
  it('returns 400 for an invalid reset token', async () => {
    ...
    const res = await app.inject({
      method: 'POST',
      url: `${BASE_URL}/password-reset/confirm`,
      payload: { resetToken: 'b'.repeat(64), newPassword: 'brand-new-password' },
    });
...
describe('POST /api/v1/auth/change-password', () => {
  it('changes the password when the current one is correct', async () => {
    ...
    const jwt = signTestToken(app, { id: 'user-1', email: 'test@example.com', role: 'user' });

    const res = await app.inject({
      method: 'POST',
      url: `${BASE_URL}/change-password`,
      headers: { authorization: `Bearer ${jwt}` },
      payload: { currentPassword: 'current-password', newPassword: 'a-new-password' },
    });
...
describe('POST /api/v1/auth/logout-all', () => {
  ...
  it('returns 401 without a token', async () => {
    const app = await buildTestApp();
    const res = await app.inject({ method: 'POST', url: `${BASE_URL}/logout-all` });
    expect(res.statusCode).toBe(401);
    await app.close();
  });
});
