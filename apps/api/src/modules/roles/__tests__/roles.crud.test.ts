...
function roleRow(over: Partial<{ id: string; name: string; description: string | null; isSystem: boolean }> = {}) {
  return {
    id: over.id ?? 'role-1',
...
function makePrisma(over: Record<string, unknown> = {}) {
  const base: Record<string, unknown> = {
    ...
    permission: {
      findMany: vi
        .fn()
        .mockResolvedValue([{ id: 'perm-1', key: PermissionKeys.UsersRead, description: 'x' }]),
    },
    rolePermission: {
...
const AUDIT = { actorId: 'actor', requestId: 'req-1', ip: '127.0.0.1', userAgent: 'test' };
...
describe('RolesService — read operations', () => {
  it('lists roles as DTOs', async () => {
    ...
    expect(roles[0]).toMatchObject({ name: 'CustomRole', permissions: [PermissionKeys.UsersRead] });
  });

  it('gets a role by id', async () => {
    const prisma = makePrisma({
      role: { findUnique: vi.fn().mockResolvedValue(roleRow({ id: 'role-1' })) },
    });
  ...
  it('throws 404 for a missing role', async () => {
    const svc = new RolesService(makePrisma());
    await expect(svc.getRole('nope')).rejects.toMatchObject({ statusCode: 404 });
  });
  ...
  it('lists permissions and user roles', async () => {
    const prisma = makePrisma({
      ...
      userRole: {
        findMany: vi.fn().mockResolvedValue([{ role: { name: SystemRoles.Support } }]),
      },
    });
    const svc = new RolesService(prisma);
...
describe('RolesService — createRole (happy path)', () => {
  it('creates a role with permissions and audits', async () => {
    ...
    const auditCreate = (prisma as unknown as { auditLog: { create: ReturnType<typeof vi.fn> } })
      .auditLog.create;

    const svc = new RolesService(prisma);
    const role = await svc.createRole(
      { name: 'Ops', description: 'ops', permissions: [PermissionKeys.UsersRead] },
  ...
  it('rejects a duplicate role name (409)', async () => {
    ...
    const svc = new RolesService(prisma);
    await expect(
      svc.createRole({ name: 'Dup' }, { userId: 'a', isSuperAdmin: true }, AUDIT)
...
describe('RolesService — updateRole / setRolePermissions', () => {
  ...
  it('blocks a non-super admin from modifying a system role (403)', async () => {
    const prisma = makePrisma({
      role: { findUnique: vi.fn().mockResolvedValue(roleRow({ id: 'r', name: 'ADMIN', isSystem: true })) },
    });
    const svc = new RolesService(prisma);
    await expect(
      svc.updateRole('r', { description: 'x' }, { userId: 'a', isSuperAdmin: false }, AUDIT)
    ).rejects.toMatchObject({ statusCode: 403 });
  });

  it('replaces a role permission set via setRolePermissions', async () => {
    const prisma = makePrisma({
      role: { findUnique: vi.fn().mockResolvedValue(roleRow({ id: 'role-1', isSystem: false })) },
    });
    const svc = new RolesService(prisma);
...
describe('RolesService — deleteRole (happy path)', () => {
  it('deletes a custom role and audits', async () => {
    const prisma = makePrisma({
      role: {
        findUnique: vi.fn().mockResolvedValue(roleRow({ id: 'role-1', isSystem: false })),
        delete: vi.fn().mockResolvedValue({ id: 'role-1' }),
      },
    });
    const svc = new RolesService(prisma);
...
describe('RolesService — setUserRoles (happy path)', () => {
  it('a super admin assigns roles, bumps permissionVersion, and audits', async () => {
    ...
    const prisma = makePrisma({
      user: { findUnique: vi.fn().mockResolvedValue({ id: 'target', email: 't@x.com' }), update: vi.fn() },
    ...
    const userUpdate = (prisma as unknown as { user: { update: ReturnType<typeof vi.fn> } }).user.update;
  ...
  it('rejects unknown role names (400)', async () => {
    ...
    const svc = new RolesService(prisma);
    await expect(
      svc.setUserRoles('target', ['NoSuchRole'], { userId: 'a', isSuperAdmin: true }, AUDIT)
