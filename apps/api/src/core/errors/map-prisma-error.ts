...
export function mapPrismaError(
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      ...
      case 'P2003':
      case 'P2014':
        return new ConflictError(`${resource} references a related record that prevents this operation`);
      default:
  ...
  if (error instanceof Prisma.PrismaClientValidationError) {
    return new AppError('Database request failed', 500, false, undefined, ErrorCode.INTERNAL_ERROR);
  }
