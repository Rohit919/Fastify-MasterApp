...
describe('API_ENDPOINTS registry', () => {
  ...
  it('composes static auth + user + admin paths correctly', () => {
    ...
    expect(API_ENDPOINTS.AUTH.RESET_PASSWORD).toBe('/api/v1/auth/password-reset/confirm');
  ...
  it('builds dynamic id paths', () => {
    ...
    expect(API_ENDPOINTS.ROLES.PERMISSIONS('r1')).toBe('/api/v1/admin/roles/r1/permissions');
    expect(API_ENDPOINTS.USER_ROLES.BY_USER('u1')).toBe('/api/v1/admin/users/u1/roles');
  });
  ...
  it('exposes Fastify route templates with :params', () => {
    ...
    expect(API_ENDPOINTS.USER_ROLES.ROUTE_BY_USER).toBe('/api/v1/admin/users/:id/roles');
  });
});
